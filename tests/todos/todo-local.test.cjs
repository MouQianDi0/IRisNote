...
test("迁移 1–6 升级到 7，保留既有表数据，迁移可重复且具备所有者日期索引", async (t) => {
    ...
    await createLocalTodos.up(database);
    // 0011 增加系统偏好表；0012 增加笔记垃圾桶表；0013 增加笔记清除标记表；0014 增加本地摘录表；0015 增加阅读进度表，当前版本随之推进。
    assert.equal(CURRENT_DATABASE_VERSION, 15);
    assert.equal(
...
test("创建幂等、无变化不增版本；并发编辑合并非重叠字段并保留完成状态", async (t) => {
    ...
    assert.deepEqual(
        await repo.complete(owner, created, false, later),
        created,
    );
    ...
    const merged = await repo.update(
        owner,
        created,
        { isStarred: true },
        later,
...
test("批量冲突全不改；第二条 SQL 失败回滚，成功后只广播一次", async (t) => {
    ...
    hooks.run = (sql) => {
        if (sql.startsWith("UPDATE") && ++calls === 2)

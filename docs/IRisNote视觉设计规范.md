...
> 文档版本：1.7（2026-09-12：草稿箱标题行加 leading 图标，提示行到按钮统一 12dp）
...
- 左上角「笔记操作」24px 常规字重。标题行：标题文字在行首占满（flex-1，最多两行），编辑图标触控区 44dp 在行尾；图标仅改标题，展开 48dp 输入框及保存/放弃修改按钮，本地先保存再同步。改名成功一律静默（编辑框收起、标题即时更新），失败错误红字显示在提示行。

...
- 账户卡片为白底、16dp 圆角、16dp 内边距。头像 56×56dp，可点击调用现有头像更换能力；头像到身份文字间距 12dp。昵称 17sp，邮箱与加入时间 13sp 蓝灰，文字按单行截断。账户加载时显示主题色指示器，未登录直达时显示明确说明并提供返回欢迎页入口。
- 账户卡片下隔 16dp 放四格状态概览卡。概览卡为白底、16dp 圆角、上下内边距 12dp；四项等宽，图标 22dp，图标到标题 8dp，标题 14sp，状态值 13sp 蓝灰。首版依次显示外观「浅色」、阅读「默认」、通知「站内」、同步「按需」；概览只陈述当前状态，不伪装成可点击入口。

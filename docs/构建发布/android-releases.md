...
按已发布构建顺序计数，预留失败、草稿和构建号跳号不增加版本距离；曾发布后撤回的版本仍计入历史。
同主版本落后 1～2 版可跳过、使用差分包；落后 3 版强制差分更新；落后超过 3 版强制下载最新完整 APK。
主版本变化（如 1.9.3 → 2.0.1）仍下载完整 APK，是否强制由相同的已发布版本距离决定。
最近三版内缺少匹配补丁、旧包摘要不符或合并失败时明确报错，不把故障当成完整包回退条件。
首次安装或为不具备差量能力的旧安装版接入本功能，需要先分发完整基础版本；
已有旧用户应通过一次主版本更新进入该基础版本。单 APK 的差量不适用于商店拆分 APK。
每次启动自动检查；同一进程内后续自动检查间隔为六小时，手动检查立即执行。
收到过的强制更新要求按当前安装构建号保存在本地，重启断网时仍显示；从未联网收到策略时不能预知服务端新版本。
强制状态沿用原弹窗布局，只保留更新按钮；下载、校验期间按钮禁用，不提供后台继续或稍后选项。
按返回键、拒绝安装授权或取消系统安装时，先保存活动草稿，再调用 Android 返回退出行为；草稿保存失败则留在弹窗中。
退出指离开当前应用界面，不强杀系统进程；重新进入仍受更新要求约束。安装失败留在弹窗重试，未设置倒计时退出。
下载依赖应用进程存活；失败可重试，
...
先部署支持 `updatePolicy=2` 的版本服务，再使用新版工具构建和发布新版客户端。
新版接口返回 `updatePolicy: { version: 2, releasesBehind, mandatory }`；客户端验证距离与强制标记一致，
只有大版本变更或明确超出三版范围才接受完整 APK。覆盖安装保留应用数据，不执行卸载或清库。
旧客户端不认识新策略且拒绝同主版本完整包：服务端为其选择仍已发布、能匹配现有差分的最新可达版本，
必要时先安装中间版本再检查更新。因此必须保留已有历史 APK/补丁；不能仅部署服务端就让旧客户端强制退出。
没有任何可达历史补丁时，旧客户端仍提示更新不可用，需要用户手动获取完整安装包。
这次变更不需要数据库迁移，不自动部署服务器或发布安装包。

### 完整包屏障（updatePolicy=3）

开发者可在预留时用 `--full-package` 把某个版本标记为“完整包屏障”：在它之前安装的用户，
升级到该版本及之后任何版本都下载完整 APK，不使用差量包；屏障之后的版本照常按最近三版规则差分。
用于差量不可靠或不合适的特殊情况（例如原生更新模块或打包方式变化），是否使用由开发者判断。

- 只能在 `reserve` 时设置，之后不能修改或撤销；标记错误时放弃该编号并重新预留。
- 只改变安装包类型，不改变是否强制：强制与否仍只由已发布版本距离决定（落后 3 版及以上强制）。
- 屏障发布后生效，撤回后仍然有效；未发布即放弃的预留不产生屏障。
- 屏障版本本身不需要差量包，`patches` 会提示无需差量；下一版只需要来自屏障及之后版本的补丁。
- 新客户端请求 `updatePolicy=3`，服务端返回 `fullPackageRequired`；为 true 时客户端接受同主版本 3 版内的完整包，
  弹窗显示“完整更新（需完整安装包）· xx MB”。未带该标记时，3 版内的完整包仍被拒绝，不作为故障回退。
- 仍请求 `updatePolicy=2` 的已安装客户端不认识屏障：本应走差量时，服务端改给屏障之前最新的已发布版本，
  强制与否按该中间版本的距离计算；装上支持策略 3 的版本后，下次检查再收到完整包。
  屏障前没有可达版本时提示“此版本需要安装完整安装包，请前往官网下载最新版本”，不弹出无法下载的强制更新。

上线顺序：服务端执行迁移 014 → 部署版本服务 → 用本工具正常发布一个支持策略 3 的客户端版本 →
之后才使用 `reserve --full-package`。旧版服务端会忽略该参数，工具在预留后核对返回记录，
未记录标记即报错停止；该编号不要继续构建发布，部署服务端后重新预留。
...
4. 将 `docs/构建发布/release.env.example` 复制为项目根目录的 `.env.release.local` 并填写参数；
   ...
   新增、删除或调整发布构建/发布工具读取的环境变量时，必须同步维护
   `docs/构建发布/release.env.example` 中的对应示例；密钥只保留空值和用途注释。
   EAS 云端凭据需预先配置同一正式签名，发布工具不会自动导出或替换签名密钥。
5. 提交应用源码及工具；构建会从指定 Git 提交导出到独立目录。
   工作区的未提交源码、忽略的 `.env.local` 和旧 `android/` 不会进入构建。
   如需自定义 API 地址，在构建进程设置两个 `EXPO_PUBLIC_*` 参数。

## 本地正式包增量构建

自有渠道的 `npm run build -- --build <构建号>` 默认使用按项目路径隔离的固定工作区，
位于 `IRIS_BUILD_ROOT`（Windows 默认 `D:\iris-build`，随项目所在盘符调整）下的 `reuse-*`。
第一次构建仍需安装依赖和完整编译；后续仅改 `src/` 内业务代码时可复用依赖与原生编译结果。
最终仍生成完整签名 APK，原有上传、差分生成、还原校验流程保持不变。

- 每次重新导出预留提交，并同步源码、删除过期文件；未变化的文件保留时间戳。
- 依赖缓存指纹包含 Node/npm 版本、npm 生效配置、平台、构建环境（不含每次递增的版本号）和 `src/` 之外的全部构建源码，包含锁文件、配置、插件、本地模块及资源。任一变化均重新安装依赖并初始化工程缓存。存在本地依赖或 workspaces 时，`src/` 也进入指纹；存在根安装生命周期脚本时，每次重新安装。
- 每次在最终路径重新生成 Android 工程，清除已移除插件的生成文件，再合并明确允许保留的 Gradle/CMake/构建输出目录。Gradle 继续自行判断任务输入变化。启用 `--daemon --build-cache`；不修改用户级 Gradle 配置，不改变 ABI、内存上限和质量检查。
- 只有 APK 身份与签名校验完成、产物复制成功后才记录可复用状态。构建中途失败，下次重新初始化；依赖安装标记缺失或变化也会使缓存失效。
- 同一项目工作区使用互斥锁。异常退出后不按 PID 自动抢锁，以免构建子进程仍在写入；必须核实子进程结束后手动解除提示中的锁，或使用 `--fresh`。
- 阶段日志包含源码导出、缓存同步、依赖安装（未命中时）、完整检查、Android 生成、Gradle 构建和 APK 校验耗时。不要把首次冷构建与后续热构建的时间混为一谈。

缓存异常或需要独立源码环境复核时：

```powershell
npm run build -- --build <构建号> --fresh
```

`--fresh` 使用全新随机目录并执行 `npm ci`，仍允许命中用户 Gradle 构建缓存，
不是“禁用所有缓存”的性能基准。EAS 继续采用全新导出目录，不使用上述本地工作区复用。
上述 build 命令沿用既有行为：自有渠道校验完成后自动上传，不能当作仅本地测速命令。
保留的工作区是受管理的缓存，不要手工修改其中的源码或依赖；怀疑其内容损坏时使用 `--fresh`。

### 构建目录自动清理

- 默认本地构建继续复用当前项目的一个 `reuse-*` 工作区，不按构建号保留多份源码、依赖和原生编译目录；这份加速缓存会保留，体积随依赖与编译结果变化。
- `--fresh` 和 EAS 使用的 `r-*` 是一次性工作区。命令正常完成或抛出构建、校验、上传错误时，在退出该流程前自动清理本次创建的目录；EAS 提前返回也会执行清理。
- 本地 APK 校验后先复制到项目 `dist/releases/<版本>/`，清理不会删除这里的正式 APK、元数据、历史差分基础包，也不会删除其他构建或项目的目录。上传失败后仍可用已保存的 APK 重试上传。
- 清理前核对目录身份与真实路径，拒绝删除被替换或指向其他位置的工作区。清理失败会输出残留路径和警告，不掩盖原始构建错误，也不会将已完成的上传伪报为构建失败。
- 强制结束进程、断电等情况可能来不及清理；旧版本遗留的 `r-*` 也不会被自动扫描删除。此类残留需核实无人使用后单独清理，不根据 PID 或目录名称猜测其安全性。
- 新逻辑只对使用更新后脚本启动的构建生效，已经运行中的构建不会因此改变行为。
...
Windows 发布构建使用项目所在盘的短目录（例如 `D:\iris-build\r-xxxxxx\source`），不再使用用户 Temp 下的长目录。可在 `.env.release.local` 设置 `IRIS_BUILD_ROOT`，必须是本地绝对路径、英文无空格且不超过 40 字符。目录自动创建；默认本地构建复用固定的 reuse-* 子目录，--fresh 和 EAS 使用本次流程结束后自动清理的 r-* 子目录。
...
# 需要强制完整包时改用：reserve ... --full-package（见“完整包屏障”）
...
上传 APK 后，工具只为同主版本、未被完整包屏障隔开的最近三个历史已发布构建（含已撤回版本）自动生成补丁，不足三个时按实际数量生成；屏障版本本身不生成补丁。
若服务端仍返回超过三个基础版本，新工具会提示先更新服务端，不继续全历史差分。
...
新版完整 APK 仍需上传并保留：供首次安装、跨主版本更新、落后超过三版的更新及后续补丁生成使用。
它不会作为最近三版内差分失败的自动备用下载。

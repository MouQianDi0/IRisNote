    ...
    const panGesture = Gesture.Pan()
        ...
        .onEnd(() => {
            if (translateX.value > OPEN_THRESHOLD) {
                translateX.value = withTiming(RIGHT_ACTION_WIDTH, {
                    duration: 180,
                });
            ...
            if (translateX.value < -OPEN_THRESHOLD) {
                translateX.value = withTiming(-LEFT_ACTION_WIDTH, {
                    duration: 180,
                });
                runOnJS(onOpenActions)(item.id);
                return;
                    ...
                    >
                        <View className="flex-row items-start justify-between gap-3">
                            ...
                            <View className="flex-row gap-1">
                                {item.is_pinned && (
                                    <PinBadge size={14} color="#2563eb" />
                                )}
                                {item.is_starred && (
                                    <StarBadge size={14} color="#f59e0b" />
                                )}
                            </View>
                        </View>
                        <Text

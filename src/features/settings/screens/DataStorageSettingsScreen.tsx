...
export default function DataStorageSettingsScreen() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      let candidates: Awaited<ReturnType<typeof readNoteCacheCandidates>> = [];
      let noteError = "";
      try {
        if (owner !== null)
      ...
      setError(
        noteError || (files.errors ? "部分占用暂未统计，可重试读取" : ""),
      );
    } catch (cause) {
      ...
      if (focused.current && version === scanVersion.current) setLoading(false);
    }
  }, [db, owner]);
  ...
  const disabled =
    loading || cleaning || !scan?.supported || owner === null || count === 0;
  const total = scan
  ...
  const summaries = [
    ...
    { label: "其他", bytes: scan?.totals.other ?? 0, color: colors.divider },
  ];
  const descriptions = {
  ...
  const clean = async () => {
    ...
    let fileResult = { released: 0, failed: 0, skipped: 0, interrupted: false };
    let noteCount = 0,
      skipped = 0;
    const errors: string[] = [];
    try {
      ...
      if (focused.current && currentOwner.current === owner) {
        ...
        if (focused.current && currentOwner.current === owner) {
          ...
          if (fileResult.failed)
            errors.push(`${fileResult.failed} 个文件未能清理，请重试`);
    ...
    <Screen className="bg-app-background">
      ...
      >
            ...
            <View key={key}>
              ...
              <Pressable
                ...
                accessibilityState={{
                  ...
                  disabled: loading || cleaning || !available[key],
                }}
                disabled={loading || cleaning || !available[key]}
                onPress={() =>
                  setSelection((value) => ({ ...value, [key]: !value[key] }))
                }
                style={{
                  flexDirection: "row",
                  alignItems: "center",
              ...
              >
                ...
                >
                  {selected[key] && <Check size={16} color={colors.surface} />}
                </View>
                <View style={{ flex: 1, gap: 4 }}>
                  <Text style={body}>{labels[key]}</Text>
                  <Text style={hint}>{values[key]}</Text>
                  <Text style={hint}>{descriptions[key]}</Text>
                </View>
              </Pressable>
            </View>
          ))}
        ...
        <Text style={{ ...hint, marginTop: 12 }}>
          ...
          {selected.notes ? `；另清理最多 ${notes.count} 条笔记缓存` : ""}
        </Text>
        ...
        <Card style={card}>
          <Detail label="更新文件总计" value={amount(scan?.totals.updates)} />
          <Detail
            label="分享临时文件总计"
      ...
      >
        ...
        >
          ...
          >
            <Text accessibilityRole="header" style={{ ...body, fontSize: 20 }}>
              清理所选项目？
            </Text>
            <Text style={{ ...hint, marginTop: 12 }}>
            ...
            <View style={{ flexDirection: "row", gap: 12, marginTop: 20 }}>
              ...
              >
                <Text style={{ ...body, color: colors.surface }}>清理</Text>
              </Pressable>
            </View>
          </View>
        </View>

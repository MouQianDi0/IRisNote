...
export const hasDraftContent = (value: NoteDraftValue) =>
    Boolean(value.title.trim() || value.content.trim());
export const newDraftKey = () =>
    "new:" +
    ...
    "-" +
    Math.random().toString(36).slice(2);
export const isNewDraftKey = (key: string) =>
    key === "new" || /^new:[a-z0-9-]+$/.test(key);
...
function parseSaved(text: string, owner: number, key: string): NoteDraft {
    ...
    if (
        ...
        !(
            ...
            typeof row.base_revision_id === "string"
        ) ||
        typeof row.base_snapshot !== "string"
    ) {
...
export async function listNewNoteDrafts(
    db: ApplicationDatabase,
    owner: number,
): Promise<DraftEntry[]> {
    const trashEnabled = await hasNoteTrash(db);
    const entries = new Map<string, DraftEntry>();
    const recovery = await db.getAll<NoteDraft>(
        "SELECT * FROM note_drafts WHERE owner_user_id = ? AND (draft_key = 'new' OR draft_key LIKE 'new:%')",
        [owner],
    );
    ...
    for (const key of await savedDraftFiles.keys(owner)) {
        ...
        if (hasDraftContent(draftValue(row)))
            entries.set(key, { key, row, kind: "saved" });
    }
...
export async function resumeNewNoteDraft(
    db: ApplicationDatabase,
    owner: number,
    key: string,
    sessionId: string,
    ...
    return db.transaction(async (tx) => {
        ...
        if (await readNoteDraft(tx, owner, key))
            throw new Error("草稿会话已变化，请重试");
        await tx.run(
...
export async function saveExplicitDraft(
    db: ApplicationDatabase,
    owner: number,
    commit: DraftCommit,
) {
...
const sameDraft = (a: NoteDraft, b: NoteDraft) =>
    (Object.keys(b) as (keyof NoteDraft)[]).every(
        (field) => a[field] === b[field],
    );

/** 草稿箱永久删除：先确保有持久恢复副本，再清文件，最后删除数据库记录。 */
export async function deleteNewNoteDrafts(
    db: ApplicationDatabase,
    owner: number,
    ...
    for (const entry of new Map(
        ...
        try {
            if (
                !isNewDraftKey(entry.key) ||
                entry.row.owner_user_id !== owner ||
                entry.row.draft_key !== entry.key
            ) {
                throw new Error("草稿身份不匹配");
            }
            const conflict = () =>
                new Error("草稿已变化，请核对刷新后的内容再删除");
            await db.transaction(async (tx) => {
                const recovery = await readNoteDraft(tx, owner, entry.key);
                if (entry.kind === "recovery") {
                    if (!recovery || !sameDraft(recovery, entry.row))
                        throw conflict();
                    return;
                }
                ...
                if (
                    text === null ||
                    !sameDraft(parseSaved(text, owner, entry.key), entry.row)
                )
                    throw conflict();
                const row = entry.row;
...
export async function discardNewRecovery(
    db: ApplicationDatabase,
    owner: number,
    commit: DraftCommit,
) {
    await db.transaction(async (tx) => {
        ...
        if (
            ...
            throw new Error("草稿会话已变化，未删除任何内容");

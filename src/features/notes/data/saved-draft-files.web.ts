...
export const savedDraftFiles = {
        ...
        try {
            ...
            for await (const name of entries.keys())
                if (name.endsWith(".json"))
                    keys.push(decodeURIComponent(name.slice(0, -5)));
            return keys;
        } catch (cause) {
    ...
    async read(owner: number, key: string): Promise<string | null> {
        try {
            ...
            ).getFileHandle(encodeURIComponent(key) + ".json");
    ...
    async write(owner: number, key: string, text: string) {
        ...
        ).getFileHandle(encodeURIComponent(key) + ".json", { create: true });
    ...
    async remove(owner: number, key: string) {
        try {
            await (
                await directory(owner)
            ).removeEntry(encodeURIComponent(key) + ".json");
        } catch (cause) {

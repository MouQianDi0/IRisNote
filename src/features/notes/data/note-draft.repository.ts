import type { ApplicationDatabase, ApplicationDatabaseTransaction } from "@/core/database";
import type { Note } from "../notes.types";

export type NoteDraftValue = { title: string; content: string; categoryId: number | null };
...
export const draftValue = (draft: NoteDraft): NoteDraftValue => ({
    title: draft.title, content: draft.content, categoryId: draft.category_id,
});
...
export function readNoteDraft(db: ApplicationDatabaseTransaction, owner: number, key: string) {
    return db.getFirst<NoteDraft>(
        "SELECT * FROM note_drafts WHERE owner_user_id = ? AND draft_key = ?", [owner, key],
    );
}

/** 新会话取得写入权；旧会话后续写入被条件更新拒绝。 */
export async function openNoteDraft(
    db: ApplicationDatabase, owner: number, key: string, sessionId: string,
    noteId: number | null, initial: NoteDraftValue, baseRevisionId: string | null = null,
) {
    return db.transaction(async (tx) => {
        await tx.run(
            ...
            [owner, key, sessionId, noteId, draftSnapshot(initial), baseRevisionId, initial.title,
                initial.content, initial.categoryId, new Date().toISOString()],
        );
...
export async function writeNoteDraft(
    db: ApplicationDatabase, owner: number, commit: DraftCommit, value: NoteDraftValue,
) {
    const startedAt = Date.now();
    const metadata = {
        database: "irisnote.db", table: "note_drafts", ownerUserId: owner,
        draftKey: commit.key, sessionId: commit.sessionId, sequence: commit.sequence,
    };
    // 不记录标题、正文、SQL 参数或异常原文；日志异常不能影响存档结果。
    const log = (event: "write_start" | "write_success" | "write_failure", reason?: string) => {
        try {
            const record = { ...metadata, event, timestamp: new Date().toISOString(),
                durationMs: Date.now() - startedAt, ...(reason ? { reason } : {}) };
    ...
    try {
        const result = await db.run(
            ...
            [value.title, value.content, value.categoryId, commit.sequence, new Date().toISOString(),
                owner, commit.key, commit.sessionId, commit.sequence],
        );
        if (result.changes !== 1) {
            reason = "session_or_sequence_mismatch";
            throw new Error("草稿会话已变化，当前输入尚未落盘。请先复制保留，再重新打开笔记");
        }
        log("write_success");
    } catch (error) {
...
export async function deleteNoteDraft(db: ApplicationDatabase, owner: number, commit: DraftCommit) {
    ...
    if (result.changes !== 1) throw new Error("草稿已变化，未删除任何内容，请重新打开后检查");
}

/** 在笔记提交事务内检查草稿身份、序号和基础内容。 */
export async function assertDraftCommit(
    tx: ApplicationDatabaseTransaction, owner: number, commit: DraftCommit | undefined,
    current: Note | null,
) {
    if (!commit) return;
    const row = await readNoteDraft(tx, owner, commit.key);
    ...
    if (current && row.base_snapshot !== draftSnapshot(noteDraftValue(current))) {
        throw new Error("已保存内容发生变化，草稿已保留，请重新打开并核对");
    }
}

export async function linkCommittedDraft(
    tx: ApplicationDatabaseTransaction, owner: number, commit: DraftCommit | undefined, note: Note,
) {
    if (!commit) return;
    await tx.run(
        `UPDATE note_drafts SET note_id = ?, base_snapshot = ?, base_revision_id = ?
         WHERE owner_user_id = ? AND draft_key = ? AND session_id = ? AND sequence = ?`,
        [note.id, draftSnapshot(noteDraftValue(note)), note.current_revision_id ?? null,
            owner, commit.key, commit.sessionId, commit.sequence],
    );
}

...
export type DraftWriteState =
    "idle" | "pending" | "writing" | "saved" | "error";
...
export class NoteDraftSession {
    ...
    get dirty() {
        return this.sequence > this.persistedSequence;
    }
    ...
    change(value: NoteDraftValue) {
        ...
        if (
            ...
            value.categoryId === this.value.categoryId
        )
            return;
        this.value = { ...value };
        this.sequence++;
        ...
        this.trailing = setTimeout(
            ...
            DRAFT_TIMING.debounceMs,
        );
        this.deadline ??= setTimeout(
            () => this.backgroundFlush(),
            DRAFT_TIMING.maxWaitMs,
        );
    }
    ...
    async beginSave() {
        if (this.closed || this.locked)
            throw new Error("当前编辑会话正在处理，请稍后再试");
        this.locked = true;
        try {
            await this.flush();
        } catch (error) {

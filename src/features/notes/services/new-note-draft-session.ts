...
export class NewNoteDraftSession {
    ...
    constructor(
        ...
        this.ready = row
            ? Promise.resolve(row)
            : openNoteDraft(db, owner, this.key, this.sessionId, null, initial);
        // 显示初始化错误由调用方负责，避免首次输入之前产生未处理拒绝。
        void this.ready.catch(() => undefined);
        this.session = new NoteDraftSession(
            row ? draftValue(row) : initial,
            row?.sequence ?? 0,
            async (value, sequence) => {
                ...
                await writeNoteDraft(
                    db,
                    owner,
                    { key: this.key, sessionId: this.sessionId, sequence },
                    value,
                );
            },
            notify,
    ...
    change(value: NoteDraftValue) {
        if (!this.done) this.session.change(value);
    }
    async flush() {
        if (!this.done) {
            await this.ready;
        ...
        try {
            ...
            return {
                ...
                commit: {
                    ...
                    beforeDelete: () =>
                        removeExplicitDraft(this.owner, this.key),
                },
            };
        } catch (cause) {
            this.unlock();
    ...
    async discard() {
        try {
            ...
            await this.ready;
            await discardNewRecovery(this.db, this.owner, {
                key: this.key,
                sessionId: this.sessionId,
                sequence,
            });

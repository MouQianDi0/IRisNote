...
function NoteViewerEditor(props: NoteViewerProps & { ownerId: number }) {
    ...
    const [initialEditPending, setInitialEditPending] = useState(
        props.initialEdit ?? false,
    );
    const beforeLeave = useCallback(
        async (snapshot: NoteDraftSaveSnapshot) => {
                    ...
                    {draft.error ? (
                        <>
                            <Text
                                ...
                                className="text-center text-hyper-error"
                            >
                                {draft.error}
                            </Text>
                            <Pressable
...
function NoteViewerSession({
    ...
    const selections = useRef({
        title: {
            start: initialValue.title.length,
            end: initialValue.title.length,
        },
        content: {
            start: initialValue.content.length,
            end: initialValue.content.length,
        },
    });
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (webBlurTimer.current !== null)
                clearTimeout(webBlurTimer.current);
        };
    }, []);
    ...
    const fieldInput = useCallback(
        (field: InlineEditField) =>
            field === "title" ? titleInput.current : contentInput.current,
        [],
    );
    ...
    const pressInput = useCallback(
        (field: InlineEditField) => {
            ...
            const result = resolveInlineEditPress(
                lastPress.current,
                field,
                performance.now(),
            );
    ...
    const blurInput = useCallback(() => {
        ...
        webBlurTimer.current = setTimeout(() => {
            ...
            if (
                !titleInput.current?.isFocused() &&
                !contentInput.current?.isFocused()
            ) {
                setKeyboardAllowed(false);
            }
        }, 0);
        ...
        (value: number) => {
            ...
            if (!Keyboard.isVisible()) {
                toolbarRestoreTimer.current = setTimeout(() => {
                    ...
                    if (!Keyboard.isVisible()) {
                        scroll.current = resetToolbarScroll(
            ...
            >
                ...
                {(draft.resource?.conflict || statusMessage) && (
                    <View className="mx-5 mt-2 flex-row items-center gap-2 rounded-2xl bg-red-50 px-3 py-2">
                        ...
                        <Text
                            accessibilityRole="alert"
                            className="flex-1 text-sm text-hyper-error"
                        >
                            {statusMessage ||
                                "本地草稿基于旧版本，请核对后确认使用"}
                        </Text>
                ...
                >
                    <Animated.ScrollView
                        ...
                        contentContainerStyle={{
                            ...
                            paddingBottom: keyboardVisible
                                ? 12
                                : FLOATING_TOUCH_HEIGHT + FLOATING_BOTTOM + 12,
                        }}
                    ...
                    >
                        <NoteViewerTitle
                            ...
                            onChangeText={(title) =>
                                changeValue({ ...latestValue.current, title })
                            }
                            onPressIn={() => pressInput("title")}
                            onFocus={() => focusInput("title")}
                            onBlur={blurInput}
                            onSelectionChange={(event) => {
                                selections.current.title =
                            ...
                            renderMeta={(titleControls) => (
                                <NoteViewerMeta
                                    ...
                                    chevronAnimatedStyle={
                                        titleControls.chevronAnimatedStyle
                                    }
                                    onToggleTitleExpanded={
                                        titleControls.onToggleTitleExpanded
                                    }
                                />
                            )}
                        />
                        <NoteViewerContent
                            ...
                            onChangeText={(content) =>
                                changeValue({ ...latestValue.current, content })
                            }
                            onPressIn={() => pressInput("content")}
                            onFocus={() => focusInput("content")}
                            onBlur={blurInput}
                            onSelectionChange={(event) => {
                                selections.current.content =
                                    event.nativeEvent.selection;
                            }}
                ...
                {keyboardVisible ? (
                    <EditorBottomToolbar
                        docked
                        disabled={draft.saving}
                        onEdit={() => requestKeyboard("content")}
                    />
                ) : (
                    <Animated.View

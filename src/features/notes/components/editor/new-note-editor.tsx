...
export default function NewNoteEditor({
    ...
    const adoptDraft = useCallback(
        async (key: string) => {
            const next = await NewNoteDraftSession.resume(
                db,
                owner,
                key,
                notify,
    ...
    const autoResume = useCallback(async () => {
        if (!draftKey || locked.current || hasDraftContent(value.current))
            return;
        locked.current = true;
        setBusy(true);
        setError("");
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            void closing
                ?.close()
                .catch(() =>
                    console.warn("[New draft] 页面卸载补写失败", { owner }),
                );
        };
            ...
            const onLocal = () => {
                ...
                if (sessionCurrent())
                    banner.show({
                        ...
                        title: "笔记已保存",
                        message:
                            cloud.enabled && cloud.ownerUserId === owner
                                ? "已保存在本机，等待同步到云端。"
                                : "已保存在本机，开启云存储后可同步。",
                    });
            };
        ...
        >
            ...
            >
                ...
                <View className="mb-3 flex-row items-center gap-1.5">
                    ...
                    <Text
                        numberOfLines={1}
                        className=".leading-[18px] flex-1 text-[13px] text-hyper-text-secondary"
                    >

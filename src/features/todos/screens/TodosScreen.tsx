...
function TodoList({
    ...
    const [deleting, setDeleting] = useState<
        readonly TodoVersionTarget[] | null
    >(null);
    ...
    useLayoutEffect(() => {
        ...
        if (target !== null) {
            ...
            if (issueScrollToDate(target, false))
                deferredScrollDateId.current = null;
            return;
        }
    ...
    useEffect(
        () => () => {
            if (scrollRetry.current.timer)
                clearTimeout(scrollRetry.current.timer);
        },
        [],
    );
            ...
            <View className="flex-1 flex-row">
                <View className="relative flex-1">
                    ...
                    <View className="relative flex-1 rounded-tr-content bg-white">
                        <View
                            className="flex-1 rounded-tr-content border-t border-r border-b border-note-page-border"
                            ...
                            >
                                <AnimatedSectionList
                                    ...
                                    onScrollToIndexFailed={() => {
                                        ...
                                        if (
                                            retry.timer ||
                                            Date.now() - retry.issuedAt >
                                                SCROLL_RETRY_WINDOW_MS ||
...
export default function TodosScreen() {
    ...
    const { reminderVisit } = useLocalSearchParams<{
        reminderVisit?: string;
    }>();
    return scope.ready ? (
        <TodoList

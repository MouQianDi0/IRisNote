...
export async function syncTodos(
    ...
    const transaction: TodoLocalRepository["syncTransaction"] = (
        owner,
        task,
    ) => {
        check();
    ...
    try {
        ...
        let cursor = await transaction(ownerKey, (tx) =>
            readCursor(tx, ownerKey),
        );
        if (cursor) {
            try {
                ...
                if (
                    ...
                    !["INVALID_CURSOR", "SYNC_CURSOR_EXPIRED"].includes(
                        error.code,
                    )
                )
                    throw error;

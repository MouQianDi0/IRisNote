...
export class TodoMemoryRepository implements TodoRepository {
  ...
  create(
    ...
    if (previous) {
      if (todoFormKeys.every((key) => previous[key] === fields[key]))
        return previous;
      throw new TodoError("duplicate", "该创建会话已经保存，请从列表重新编辑");
    }
  ...
  private resolveTargets(
    ...
    this.assertOwner(ownerKey);
    if (
      new Set(targets.map((target) => target.clientId)).size !== targets.length
    )
    ...
    return targets.map((target) => {
      ...
      if (entity.localVersion !== target.localVersion)
        throw new TodoError("conflict", "所选待办已变化，请取消确认后重新选择");
      return entity;
    });
  }
  ...
  batch(
    ...
    if (candidates.every((entity, index) => entity === current[index])) return;
    for (const entity of candidates) this.entities.set(entity.clientId, entity);
    this.broadcast();
  }

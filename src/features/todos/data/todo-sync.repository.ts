...
const decode = (row: Row): TodoSyncRecord => ({
  ...
  remote: row.remote_json ? (JSON.parse(row.remote_json) as TodoRemote) : null,
  status: row.status,
  error: row.error,
});
...
async function publish(tx: Tx, s: TodoSyncRecord, remote: TodoRemote) {
  if (isDeleted(remote)) {
    await tx.run("DELETE FROM local_todos WHERE owner_key=? AND client_id=?", [
      s.ownerKey,
      s.clientId,
    ]);
    s.deleted = true;
    ...
    if (!op) {
      ...
      if (!s.base)
        ...
        if (
          ...
          request = {
            kind: keys[0] === "is_starred" ? "set_starred" : "set_pinned",
            id: s.base.id,
            expected_version: s.base.version,
...
export async function acceptOperation(
  ...
  s.remote = newest(s.remote, remote);
  s.base = remote;
  await tx.run("DELETE FROM todo_outbox WHERE owner_key=? AND operation_id=?", [
    op.owner_key,
    op.operation_id,
  ]);
...
export async function resolveTodoConflict(
  ...
  if (
    ...
    throw new TodoError("conflict", "待办或云端版本已变化，请重新打开冲突处理");

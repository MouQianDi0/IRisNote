...
export class TodoLocalRepository implements TodoRepository {
    ...
    private async mutate<T>(
        ...
        return this.enqueue(async () => {
            ...
            const committed = await database.transaction(
                async (transaction) => {
                    ...
                    const nextIds = new Set(
                        after.map((entity) => entity.clientId),
                    );
                    for (const entity of after) {
                        ...
                        if (!old) {
                            await transaction.run(
                                `INSERT INTO local_todos (${columns}) VALUES (${values(
                                    entity,
                                )
                                    .map(() => "?")
                                    .join(", ")})`,
            ...
            if (
                ...
                entities.some(
                    (entity, index) => entity !== this.entities[index],
                )
            ) {
                this.entities = Object.freeze(entities);
                this.broadcast();
    ...
    syncTransaction<T>(
        ...
        return this.enqueue(async () => {
            ...
            this.assertSession(ownerKey, generation);
            const prior = new Map(
                this.entities.map((item) => [item.clientId, item]),
            );

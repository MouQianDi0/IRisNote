...
function CheckCircle({ checked, color }: { checked: boolean; color: string }) {
  return (
    ...
    >
      {checked && <Check size={14} color={semanticColors.onBrandPrimary} />}
    </View>
  );
}
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
            ...
            <Text
              ...
              style={{
                fontSize: 17,
                color: titleColor,
                textDecorationLine: state === "ended" ? "line-through" : "none",
                flexShrink: 1,
              }}
            >
              {todoTitle(todo)}
          ...
          >
            ...
            <Text
              numberOfLines={1}
              style={{ fontSize: 13, color: titleColor, flexShrink: 1 }}
            >
              {todoTimeLabel(todo)}
            </Text>
          </View>
        ...
        <Pressable
          ...
          accessibilityState={{ checked: todo.isCompleted, disabled: batch }}
          disabled={batch}
          onPress={(event) => {
        ...
        >
          <CheckCircle checked={todo.isCompleted} color={pair.accent} />
        </Pressable>
      </View>
    </Pressable>
  );

...
export const applicationDatabaseResource = new SharedDatabaseResource(
    async () => {
        ...
        try {
            ...
            if (
                __DEV__ &&
                process.env.EXPO_PUBLIC_DATABASE_DIAGNOSTICS ===
                    DATABASE_DIAGNOSTICS_ENABLED_VALUE
            ) {
            ...
            try {
                ...
                console.error(
                    "[Database] Failed to close initialization connection.",
                );
            }
            throw error;
        }

...
function assertDiagnosticDatabaseNamesAreSafe() {
    const names = [DIAGNOSTIC_DATABASE_NAME, DIAGNOSTIC_BACKUP_DATABASE_NAME];
    if (
...
export async function runDatabaseDiagnostics(): Promise<DatabaseDiagnosticReport> {
    ...
    try {
        await runLifecycleDiagnostics((name, passed) =>
            addCheck(checks, name, passed),
        );
        source = await openDatabaseAsync(DIAGNOSTIC_DATABASE_NAME);
        peer = await openDatabaseAsync(DIAGNOSTIC_DATABASE_NAME, {
        ...
        await port.run(
            "CREATE TABLE diagnostic_parents (id INTEGER PRIMARY KEY)",
        );
        ...
        await port.transaction(async (transaction) => {
            const enabled = await transaction.getFirst<{
                foreign_keys: number;
            }>("PRAGMA foreign_keys");
            addCheck(
                checks,
                "transaction-foreign-keys-enabled",
                enabled?.foreign_keys === 1,
            );
            await transaction.run(
                "INSERT INTO diagnostic_parents (id) VALUES (1)",
            );
            await transaction.run(
                "INSERT INTO diagnostic_children (id, parent_id) VALUES (1, 1)",
            );
        });
        let foreignKeyRejected = false;
        try {
            await port.transaction((transaction) =>
                transaction.run(
        ...
        await port.transaction((transaction) =>
            transaction.run("DELETE FROM diagnostic_parents WHERE id = 1"),
        );
        const child = await port.getFirst(
            "SELECT id FROM diagnostic_children LIMIT 1",
        );
        addCheck(checks, "transaction-foreign-key-cascade", child === null);

        // 关闭请求到达时已有事务和查询排队；全部完成后才允许底层关闭。
        const accepted = port.transaction((transaction) =>
            transaction.run(
                "INSERT INTO diagnostic_records (value) VALUES (?)",
                [CLOSE_MARKER],
            ),
        ...
        const lateRejected = port.getFirst("SELECT 1").then(
            () => false,
            () => true,
        );
        ...
        addCheck(
            ...
            (await queuedRead)?.value === CLOSE_MARKER,
        );
        managed = null;
        source = await openDatabaseAsync(DIAGNOSTIC_DATABASE_NAME);
        ...
        addCheck(
            checks,
            "close-drained-write-persists",
            savedBeforeClose?.value === CLOSE_MARKER,
        );

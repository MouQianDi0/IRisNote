...
import { create, getAdapter, type AxiosRequestConfig, type GenericAbortSignal } from "axios";
import {
...
const finishCloudRequest = (config: object | undefined) => {
  ...
  try {
    ...
    const failure = new CloudStoragePermissionError("云存储授权或账号已变化，本次传输已停止", request.dispatched);
...
function stampCloudRequest(config: AxiosRequestConfig, url?: string, method?: string): AxiosRequestConfig {
  const request = { ...config, ...(url === undefined ? {} : { url }), ...(method === undefined ? {} : { method }) };
  if (isEssentialAccountRequest(request.method, request.url)) return request;
  assertCloudStorageAllowed();
  const state = getCloudStorageSnapshot();
  return {
    ...request,
    cloudStorageContext: { ownerUserId: state.ownerUserId!, generation: state.generation },
  };
}

function stampRequestArguments(args: unknown[]): unknown[] {
  return typeof args[0] === "string"
    ? [args[0], stampCloudRequest((args[1] as AxiosRequestConfig | undefined) ?? {}, args[0])]
    : [stampCloudRequest((args[0] as AxiosRequestConfig | undefined) ?? {})];
}
...
const api = new Proxy(transport, {
  apply(target, thisArg, args) {
    try { return Reflect.apply(target, thisArg, stampRequestArguments(args)); }
  ...
  get(target, property, receiver) {
    ...
    if (property === "request") {
      return (...args: unknown[]) => {
        try { return Reflect.apply(value, target, stampRequestArguments(args)); }
        catch (error) { return Promise.reject(error); }
      };
    }
    if (["get", "delete", "head", "options", "post", "put", "patch", "postForm", "putForm", "patchForm"].includes(property)) {
      return (...args: unknown[]) => {
        try {
          const method = property.replace("Form", "");
          const configIndex = ["post", "put", "patch"].includes(method) ? 2 : 1;
          const stamped = [...args];
          stamped[configIndex] = stampCloudRequest((args[configIndex] as AxiosRequestConfig | undefined) ?? {}, args[0] as string, method);
...
api.interceptors.request.use(async (config) => {
  try {
    if (!isEssentialAccountRequest(config.method, config.url)) {
      ...
      const state = getCloudStorageSnapshot();
      if (!context || context.generation !== state.generation || context.ownerUserId !== state.ownerUserId) {
      ...
      cloudRequests.set(config, { permission, originalSignal, abort, dispatched: false });
    }
    ...
    if (cloudRequest) {
      ...
      config.adapter = async (requestConfig) => {
        try {
          ...
          if (isCloudStoragePermissionError(error)) error.config = requestConfig;
          throw error;
        }
      };
    }

...
export function cosConfig(env = process.env) {
    const required = (name) => {
        ...
        if (!value)
            throw new Error(
                `缺少 ${name}，请在 .env.release.local 配置 COS 上传凭据`,
            );
        return value;
    };
...
export function withoutCosCredentials(env) {
    return Object.fromEntries(
        Object.entries(env).filter(
            ([key]) => !/^(IRIS_COS_|COS_|TENCENTCLOUD_)/i.test(key),
        ),
    );
}
...
export function cosObjectKey(config, release) {
    if (
        ...
        !Number.isSafeInteger(release.build_code) ||
        release.build_code < 1
    )
...
export function createCosUploader(
    ...
    async function preflight() {
        let result;
        try {
            result = await client.getBucketVersioning(bucket);
        } catch (error) {
        ...
        if (status && status !== "Enabled")
            throw new Error(
                "COS 版本控制已暂停或状态无法识别；请核对配置，脚本不会更改桶设置",
            );
    ...
    async function verify(params, info, metadata, url) {
        if (
            !metadata ||
            Number(metadata.headers?.["content-length"]) !== info.size
        )
            throw new Error(
                "COS 同名对象大小不一致或不存在，拒绝覆盖/认定上传成功",
            );
        if (!metadata.ETag)
            throw new Error("COS 未返回 ETag，无法校验 CDN 对应对象");
        if (
            metadata.headers?.["x-cos-meta-sha256"] &&
            metadata.headers["x-cos-meta-sha256"] !== info.sha256
        )
        ...
        try {
            ...
            throw new Error("CDN 回读请求失败或超时，APK 保留，可重试 upload");
        }
        if (
            response.status !== 200 ||
            !response.body ||
            ...
            throw new Error(
                `CDN 回读不匹配（HTTP ${response.status}、大小或 ETag）；请检查 CDN 回源/缓存后重试，不会覆盖 COS 对象`,
            );
        }
        try {
            ...
            throw new Error("CDN 回读中断或超出预期大小，可重试 upload");
        }
        ...
        if (!sameObject(metadata, await head(params)))
            throw new Error(
                "COS 对象在校验期间发生变化，拒绝认定上传成功；请检查其他上传进程",
            );
    }
    async function upload(apk, release, info) {
        ...
        if (
            uploaded &&
            (!uploaded.ETag ||
                uploaded.ETag !== metadata?.ETag ||
                (versioning === "Enabled" &&
                    (!uploaded.VersionId ||
                        uploaded.VersionId !== metadata?.VersionId)))
        )
            throw new Error(
                "COS 当前对象与本次上传的 ETag/版本 ID 不一致；可能存在并发写入，请核对后重试",
            );

...
const excludedRoots = new Set([
    ...
    "待办事项.md",
    "debug.log",
    "tmpwebapp-node-modulesprepare.log",
]);
...
export function exportBuildSource(root, commit, workspace, checkout) {
    const entries = run("git", ["ls-tree", "--name-only", "-z", commit], {
        cwd: root,
        capture: true,
    })
        .split("\0")
    ...
    if (
        !included.includes("package.json") ||
        !included.includes("package-lock.json")
    ) {
    ...
    return {
        included,
        excluded: entries.filter((name) => !includeBuildRoot(name)),
    };
}

...
export async function gradleEnvironment(
    projectRoot,
    ...
    const socketDirectory = path.resolve(
        environment.IRIS_GRADLE_SOCKET_DIR || path.join(projectRoot, ".expo"),
    );
    if (/["\r\n]/.test(socketDirectory))
        throw new Error("Gradle socket 路径不能包含引号或换行");
    // Unix-domain sockaddr paths are limited; leave room for socket_<random>.
    if (
        new TextEncoder().encode(
            path.join(socketDirectory, "socket_2147483647"),
        ).length > 100
    ) {
        throw new Error(
            "Gradle socket 路径过长，请通过 IRIS_GRADLE_SOCKET_DIR 指定可写的短路径",
        );
    }
    ...
    result.JAVA_TOOL_OPTIONS = original.includes(property)
        ? original
        : [original, property].filter(Boolean).join(" ");
    return result;
}
